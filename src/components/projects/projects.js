                                                ...
                                                <div className={`${localStorage.getItem('auth') ? JSON.parse(localStorage.getItem('auth')).role === 1 ? 'space-between' : "float-right" : "float-right"}`}>
                                                    {
                                                        localStorage.getItem('auth') ? JSON.parse(localStorage.getItem('auth')).role === 1 ?
                                                            <> <button className="btn-danger" id={item.projid} onClick={deleteProject}>delete</button><Link to={`/projects/tasks/${item.projid}`} state={{prjectName: item.name}} className="btn-default float-right"><FontAwesomeIcon icon={faListCheck}></FontAwesomeIcon> tasks</Link></> : <Link to={`/projects/tasks/${item.projid}`} state={{prjectName: item.name}} className="btn-default float-right"><FontAwesomeIcon icon={faListCheck}></FontAwesomeIcon>tasks</Link> : <Link to={`/projects/tasks/${item.projid}`}  state={{prjectName: item.name}} className="btn-default float-right"><FontAwesomeIcon icon={faListCheck}></FontAwesomeIcon>tasks</Link>
